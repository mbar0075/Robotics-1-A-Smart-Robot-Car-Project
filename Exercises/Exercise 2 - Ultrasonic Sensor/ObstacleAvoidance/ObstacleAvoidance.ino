...
void scan_and_evade_obstacle()
{
  ...
  */
  ...
  // Scanning environment to get the angle where there is no obstacle.
  int obstacle_free_angle = 0;

  // Using the servo to scan the environment for obstacles.
  for (int i = 0; i < 180; i += 30)
  {
    // Setting the servo to the current angle.
    ...
    if (!is_obstacle(20))
    {
      // Storing the angle where there is no obstacle.
      obstacle_free_angle = i;
  ...
  // Reverse the robot.
  bwd(get_speed(100), get_speed(100));
  delay(700);

  // Turning the robot depending on the angle where there is no obstacle.
  if (obstacle_free_angle < 90)
  {
    // Right turn.
    right_bwd(get_speed(200));
    delay(1000);
  } 
  else if (obstacle_free_angle > 90)
  {
    // Left turn.
    left_bwd(get_speed(200));
    delay(1000);
  }
...
void obstacle_avoidance()
{
  ...
  */
  ...
  if (is_obstacle(20))
  {
    ...
    fwd(get_speed(100), get_speed(100));
  }
