...
void loop() {
  ...
  // Making the robot form a square.
  
  // Looping for 4 times to make the robot move in a square.
  for (int i = 0; i < 4; i++)
  {
      // Moving the robot forward.
      fwd(get_speed(70), get_speed(70));
      delay(3000);

      // Stopping the robot for 4 seconds.
      stop_move();
      delay(3000);
      
      // Making the robot turn exactly 90 degrees to the right.
      right_fwd(get_speed(100));
      delay(1250); // Adjust this value to make sure the robot turns 90 degrees accurately.
      stop_move(); // Stop the turn.
      delay(1000); // Adjust this value based on your robot's behavior.
  }

  // Turn the LED on, wait 5 seconds, then turn it off.
