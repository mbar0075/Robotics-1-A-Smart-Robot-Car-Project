...
void loop() {
  ...
  // Calling the wall following function.
  wall_following();
}
...
void wall_following()
{
  /* Function to follow the wall and corners.
  */
  // If there is an obstacle in front of the robot (wall then the robot will follow the wall).
  if (is_obstacle(distance_threshold))
  {
    // Going forward.
    fwd(current_speed, current_speed);

  }
  else
  {
    // If there is no obstacle in front of the robot, stop scan the nearby area for angles 30, 90, 150 and if there is a wall at any of these angles, turn towards that angle.
      for (uint8_t i = 1; i < 6; i += 2) //1、3、5 Omnidirectional detection of obstacle avoidance status
    {
      ...
      if (is_obstacle(distance_threshold))
      {
        // Switch statement to turn the robot towards the angle where the obstacle is detected.
        switch (i)
        {
        case 1:
          // Going Right.
